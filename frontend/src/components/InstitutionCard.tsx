...
function InstitutionCard({
  ...
  const hoursText =
    institution.todayOpenTime && institution.todayCloseTime
  ...
  const hasEmergencyAvailability =
    isEmergencyRoom && institution.availableEmergencyBeds !== null;

  return (
    <article
      className={
        selected
          ? `institution-card ${typeClass} is-selected`
          : `institution-card ${typeClass}`
      }
      onClick={onSelect}
    >
      <div className="institution-type-icon">
        <TypeIcon size={25} />
      </div>
      <div className="institution-main">
        <div className="institution-title-row">
          <h2>{institution.name}</h2>
          <span className="type-badge" title={typeBadgeLabel}>
            {typeBadgeLabel}
          </span>
          ...
          {!isEmergencyRoom &&
            (institution.operatingHoursKnown ? (
              <span
                className={institution.open ? "open-badge" : "closed-badge"}
              >
        ...
        <p className="institution-address">
          {institution.roadAddress ?? "주소 정보 없음"}
        </p>
      ...
      <div className="hours-box">
        ...
        <strong
          className={
            hasEmergencyAvailability && institution.availableEmergencyBeds === 0
              ? "no-beds"
              : ""
          }
        >
      ...
      <div className="card-actions">
        {user && (
          <button
            className={
              favorite ? "favorite-button is-active" : "favorite-button"
            }
            type="button"
